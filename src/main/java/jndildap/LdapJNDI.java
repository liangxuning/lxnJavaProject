...
//        String username = "uid=USER_NAME,ou=Authorization,ou=People,o=cc.com,o=isp";//xxx为申请的对接账户
        String username = "cn=Manager,dc=my-domain,dc=com";//xxx为申请的对接账户
        String password = "123";

        Hashtable env = new Hashtable();
        env.put(Context.INITIAL_CONTEXT_FACTORY, "com.sun.jndi.ldap.LdapCtxFactory");//设置连接LDAP的实现工厂
        env.put(Context.PROVIDER_URL, "ldap://10.31.2.45:389/");// 指定LDAP服务器的主机名和端口号
...
//            Attributes attrs = ctx.getAttributes("uid=00012047,ou=Internal,ou=People");//获取到一个人员，
            ...
            NamingEnumeration bindings = ctx.listBindings("cn=OMC-R,dc=my-domain,dc=com");//列举 内部人员
            NamingEnumeration<NameClassPair> nnn = ctx.list("cn=OMC-R,dc=my-domain,dc=com");
            while (nnn.hasMore()) {
                NameClassPair o = nnn.next();
